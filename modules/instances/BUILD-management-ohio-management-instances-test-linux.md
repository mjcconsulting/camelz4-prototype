# Modules:Instances:Management Account:Ohio:Management Linux Test Instances

This module creates Management Linux Test Instances in the Management VPC in the AWS Virginia (us-east-2) Region
...
- Public Hosted Zone: us-east-2.camelz.io
- KeyPair: administrator
- VPC: (Ohio) Management-VPC

## Ohio Management Linux Test Instances
...
1. **Create LinuxWebServer Identifiers**
    ...
    ```bash
    ohio_management_lws_instancea_description="CaMeLz Ohio Management Linux Web Server 01-A"
    camelz-variable ohio_management_lws_instancea_description

    ohio_management_lws_instancea_public_hostname=cmlue1mlws01a.$ohio_management_public_domain
    camelz-variable ohio_management_lws_instancea_public_hostname

    ohio_management_lws_instancea_public_hostalias=lwsa.$ohio_management_public_domain
    camelz-variable ohio_management_lws_instancea_public_hostalias

    ohio_management_lws_instancea_private_hostname=cmlue1mlws01a.$ohio_management_private_domain
    camelz-variable ohio_management_lws_instancea_private_hostname

    ohio_management_lws_instancea_private_hostalias=lwsa.$ohio_management_private_domain
    camelz-variable ohio_management_lws_instancea_private_hostalias
    ```
...
1. **Create LinuxWebServer Security Group**
    ...
    camelz-variable ohio_management_lws_sg_id
...
1. **Create LinuxWebServer EIP**
    ...
    ohio_management_lws_eipa=$(aws ec2 allocate-address --domain vpc \
                                                        ...
                                                        --query 'AllocationId' \
                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_lws_eipa

    ohio_management_lws_instancea_public_ip=$(aws ec2 describe-addresses --allocation-ids $ohio_management_lws_eipa \
                                                                         --query 'Addresses[0].PublicIp' \
                                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_lws_instancea_public_ip
    ```

1. **Create LinuxWebServer Public Domain Name**

    ```bash
    tmpfile=$CAMELZ_HOME/tmp/ohio-management-lwsa-public-$$.json
    sed -e "s/@hostname@/$ohio_management_lws_instancea_public_hostname/g" \
        -e "s/@servicename@/$ohio_management_lws_instancea_public_hostalias/g" \
        -e "s/@ip@/$ohio_management_lws_instancea_public_ip/g" \
...
1. **Create LinuxWebServer Instance**
    ...
    ```bash
    tmpfile=$CAMELZ_HOME/tmp/ohio-management-lwsa-user-data-$$.sh
    sed -e "s/@hostname@/$ohio_management_lws_instancea_private_hostname/g" \
        -e "s/@motd@/$ohio_management_lws_instancea_description/g" \
    ...
    ohio_management_lws_instancea_id=$(aws ec2 run-instances --image-id $ohio_amzn2_ami_id \
                                                             ...
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_lws_instancea_id
    ...
    ohio_management_lws_instancea_private_ip=$(aws ec2 describe-instances --instance-ids $ohio_management_lws_instancea_id \
                                                                          ...
                                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_lws_instancea_private_ip
    ```

1. **Create LinuxWebServer Private Domain Name**

    ```bash
    tmpfile=$CAMELZ_HOME/tmp/ohio-management-lwsa-private-$$.json
    sed -e "s/@hostname@/$ohio_management_lws_instancea_private_hostname/g" \
        -e "s/@servicename@/$ohio_management_lws_instancea_private_hostalias/g" \
        -e "s/@ip@/$ohio_management_lws_instancea_private_ip/g" \
        $CAMELZ_HOME/templates/route53-upsert-a-hostname-cname-servicename.json > $tmpfile
...
1. **Associate LinuxWebServer EIP with Instance & Create Reverse DNS Public Domain Name**
    ...
    aws ec2 modify-address-attribute --allocation-id $ohio_management_lws_eipa \
                                     --domain-name $ohio_management_lws_instancea_public_hostname \
                                     --profile $profile --region us-east-2 --output text
    ```
...
1. **Create LinuxApplicationServer Identifiers**
    ...
    ```bash
    ohio_management_las_instancea_description="CaMeLz Ohio Management Linux Application Server 01-A"
    camelz-variable ohio_management_las_instancea_description

    ohio_management_las_instancea_private_hostname=cmlue1mlas01a.$ohio_management_private_domain
    camelz-variable ohio_management_las_instancea_private_hostname

    ohio_management_las_instancea_private_hostalias=lasa.$ohio_management_private_domain
    camelz-variable ohio_management_las_instancea_private_hostalias
    ```
...
1. **Create LinuxApplicationServer Security Group**
    ...
    ohio_management_las_sg_id=$(aws ec2 create-security-group --group-name Management-LinuxApplicationServer-InstanceSecurityGroup \
                                                              ...
                                                              --query 'GroupId' \
                                                              --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_las_sg_id
...
1. **Create LinuxApplicationServer Instance**
    ...
    ```bash
    tmpfile=$CAMELZ_HOME/tmp/ohio-management-lasa-user-data-$$.sh
    sed -e "s/@hostname@/$ohio_management_las_instancea_private_hostname/g" \
        -e "s/@motd@/$ohio_management_las_instancea_description/g" \
        $CAMELZ_HOME/templates/linux-standard-user-data.sh > $tmpfile
    ...
    ohio_management_las_instancea_id=$(aws ec2 run-instances --image-id $ohio_amzn2_ami_id \
                                                             ...
                                                             --query 'Instances[0].InstanceId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_las_instancea_id

    ohio_management_las_instancea_private_ip=$(aws ec2 describe-instances --instance-ids $ohio_management_las_instancea_id \
                                                                          --query 'Reservations[0].Instances[0].PrivateIpAddress' \
                                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_las_instancea_private_ip
    ```

1. **Create LinuxApplicationServer Private Domain Name**

    ```bash
    tmpfile=$CAMELS_HOME/tmp/ohio-management-lasa-private-$$.json
    sed -e "s/@hostname@/$ohio_management_las_instancea_private_hostname/g" \
        -e "s/@servicename@/$ohio_management_las_instancea_private_hostalias/g" \
        -e "s/@ip@/$ohio_management_las_instancea_private_ip/g" \
        $CAMELZ_HOME/templates/route53-upsert-a-hostname-cname-servicename.json > $tmpfile
