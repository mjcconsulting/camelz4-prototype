...
1. **Configure Sub-Domain Name Servers in the Parent Hosted Zone**
    ...
    ```bash
    tmpfile=$CAMELZ_HOME/tmp/oregon-production-ns-$$.json
    sed -e "s/@name@/$oregon_production_public_domain/g" \
...
1. **Create Check TXT Record in Public Hosted Zone and Confirm**
   ...
   properly setup in the public DNS hierarchy.
    ...
    tmpfile=$CAMELZ_HOME/tmp/oregon-production-txt-check-$$.json
    sed -e "s/@name@/check.$oregon_production_public_domain/g" \
        -e "s/@value@/$value/g" \
        $CAMELZ_HOME/templates/route53-upsert-txt.json > $tmpfile

    aws route53 change-resource-record-sets --hosted-zone-id $oregon_production_public_hostedzone_id \
                                            --change-batch file://$tmpfile \
                                            --profile $profile --region us-east-1 --output text

    sleep 15

    [ "$(dig +short -t TXT check.$oregon_production_public_domain)" = "\"$value\"" ] && echo "Check confirmed" || echo "Check failed"
    ```
