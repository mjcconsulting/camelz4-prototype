# Modules:VPCs:Alfa Production Account:Ohio:Alfa Production VPC
...
1. **Create VPC**
    ...
    camelz-variable alfa_ohio_production_vpc_id
...
1. **Tag Attached Default Resources Created With VPC**
    ...
    main_rtb_id=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$alfa_ohio_production_vpc_id \
    ...
    default_nacl_id=$(aws ec2 describe-network-acls --filters Name=vpc-id,Values=$alfa_ohio_production_vpc_id \
    ...
    default_sg_id=$(aws ec2 describe-security-groups --filters Name=vpc-id,Values=$alfa_ohio_production_vpc_id \
...
1. **Create VPC Flow Log**
    ...
    ```bash
    aws logs create-log-group --log-group-name "/$company_name_lc/$system_name_lc/FlowLog/Alfa-Production/Ohio" \
                              --profile $profile --region us-east-2 --output text

    aws ec2 create-flow-logs --resource-type VPC --resource-ids $alfa_ohio_production_vpc_id \
                             --traffic-type ALL \
                             --log-destination-type cloud-watch-logs \
                             --log-destination "arn:aws:logs:us-east-2:${alfa_production_account_id}:log-group:/${company_name_lc}/${system_name_lc}/FlowLog/Alfa-Production/Ohio" \
...
1. **Create Internet Gateway**
    ...
    alfa_ohio_production_igw_id=$(aws ec2 create-internet-gateway --tag-specifications "ResourceType=internet-gateway,Tags=[{Key=Name,Value=Alfa-Production-InternetGateway},{Key=Company,Value=Alfa},{Key=Environment,Value=Production},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                                  ...
                                                                  --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_igw_id
...
1. **Create Private Hosted Zone**
    ...
    camelz-variable alfa_ohio_production_private_hostedzone_id
    ```
...
1. **Create DHCP Options Set**
    ...
    alfa_ohio_production_dopt_id=$(aws ec2 create-dhcp-options --dhcp-configurations "Key=domain-name,Values=[$alfa_ohio_production_private_domain]" \
                                                               ...
                                                               --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_dopt_id
...
1. **Create Public Subnet A**
    ...
    alfa_ohio_production_public_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                   ...
                                                                   --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_public_subneta_id
    ```
...
1. **Create Public Subnet B**
    ...
    alfa_ohio_production_public_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                   ...
                                                                   --query 'Subnet.SubnetId' \
                                                                   --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_public_subnetb_id
    ```
...
1. **Create Web Subnet A**
    ...
    alfa_ohio_production_web_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_web_subneta_id
    ```
...
1. **Create Web Subnet B**
    ...
    alfa_ohio_production_web_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_web_subnetb_id
    ```
...
1. **Create Application Subnet A**
    ...
    alfa_ohio_production_application_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                        ...
                                                                        --query 'Subnet.SubnetId' \
                                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_application_subneta_id
    ```
...
1. **Create Application Subnet B**
    ...
    alfa_ohio_production_application_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                        ...
                                                                        --query 'Subnet.SubnetId' \
                                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_application_subnetb_id
    ```
...
1. **Create Database Subnet A**
    ...
    alfa_ohio_production_database_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                     ...
                                                                     --query 'Subnet.SubnetId' \
                                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_database_subneta_id
    ```
...
1. **Create Database Subnet B**
    ...
    alfa_ohio_production_database_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                     ...
                                                                     --query 'Subnet.SubnetId' \
                                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_database_subnetb_id
    ```
...
1. **Create Directory Subnet A**
    ...
    alfa_ohio_production_directory_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                      ...
                                                                      --query 'Subnet.SubnetId' \
                                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_directory_subneta_id
    ```
...
1. **Create Directory Subnet B**
    ...
    alfa_ohio_production_directory_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                      ...
                                                                      --query 'Subnet.SubnetId' \
                                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_directory_subnetb_id
    ```
...
1. **Create Endpoint Subnet A**
    ...
    alfa_ohio_production_endpoint_subneta_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                     ...
                                                                     --query 'Subnet.SubnetId' \
                                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_endpoint_subneta_id
    ```
...
1. **Create Endpoint Subnet B**
    ...
    alfa_ohio_production_endpoint_subnetb_id=$(aws ec2 create-subnet --vpc-id $alfa_ohio_production_vpc_id \
                                                                     ...
                                                                     --query 'Subnet.SubnetId' \
                                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_endpoint_subnetb_id
    ```
...
1. **Create Public Route Table, Default Route and Associate with Public Subnets**
    ...
    alfa_ohio_production_public_rtb_id=$(aws ec2 create-route-table --vpc-id $alfa_ohio_production_vpc_id \
                                                                    ...
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_public_rtb_id
...
1. **Create NAT Gateways - OR - NAT Instances**
    ...
    if [ $use_ngw = 1 ]; then
      ...
      alfa_ohio_production_ngw_eipa=$(aws ec2 allocate-address --domain vpc \
                                                               ...
                                                               --query 'AllocationId' \
                                                               --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_ngw_eipa
      ...
      alfa_ohio_production_ngwa_id=$(aws ec2 create-nat-gateway --allocation-id $alfa_ohio_production_ngw_eipa \
                                                                ...
                                                                --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_ngwa_id
      ...
      if [ $ha_ngw = 1 ]; then
        alfa_ohio_production_ngw_eipb=$(aws ec2 allocate-address --domain vpc \
                                                                 ...
                                                                 --query 'AllocationId' \
                                                                 --profile $profile --region us-east-2 --output text)
        camelz-variable alfa_ohio_production_ngw_eipb
        ...
        alfa_ohio_production_ngwb_id=$(aws ec2 create-nat-gateway --allocation-id $alfa_ohio_production_ngw_eipb \
                                                                  ...
                                                                  --query 'NatGateway.NatGatewayId' \
                                                                  --profile $profile --region us-east-2 --output text)
        camelz-variable alfa_ohio_production_ngwb_id
        ...
        alfa_ohio_production_ngw_eipc=$(aws ec2 allocate-address --domain vpc \
                                                                 ...
                                                                 --query 'AllocationId' \
                                                                 --profile $profile --region us-east-2 --output text)
        camelz-variable alfa_ohio_production_ngw_eipc
        ...
        alfa_ohio_production_ngwc_id=$(aws ec2 create-nat-gateway --allocation-id $alfa_ohio_production_ngw_eipc \
                                                                  ...
                                                                  --query 'NatGateway.NatGatewayId' \
                                                                  --profile $profile --region us-east-2 --output text)
        camelz-variable alfa_ohio_production_ngwc_id
      fi
    else
      ...
      alfa_ohio_production_nat_sg_id=$(aws ec2 create-security-group --group-name Alfa-Production-NAT-InstanceSecurityGroup \
                                                                     ...
                                                                     --query 'GroupId' \
                                                                     --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_nat_sg_id
      ...
      alfa_ohio_production_nat_instance_id=$(aws ec2 run-instances --image-id $ohio_nat_ami_id \
                                                                   ...
                                                                   --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=Alfa-Production-NAT-Instance},{Key=Hostname,Value=cmlue1mnat01a},{Key=Company,Value=Alfa},{Key=Environment,Value=Production},{Key=Utility,Value=NAT},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                                   --query 'Instances[0].InstanceId' \
                                                                   --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_nat_instance_id
      ...
      alfa_ohio_production_nat_instance_eni_id=$(aws ec2 describe-instances --instance-ids $alfa_ohio_production_nat_instance_id \
                                                                            ...
                                                                            --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_nat_instance_eni_id
      ...
      alfa_ohio_production_nat_instance_private_ip=$(aws ec2 describe-instances --instance-ids $alfa_ohio_production_nat_instance_id \
                                                                                ...
                                                                                --profile $profile --region us-east-2 --output text)
      camelz-variable alfa_ohio_production_nat_instance_private_ip
    fi
    ```
...
1. **Create Private Route Table for Availability Zone A, Default Route and Associate with Private Subnets**
    ...
    alfa_ohio_production_private_rtba_id=$(aws ec2 create-route-table --vpc-id $alfa_ohio_production_vpc_id \
                                                                      ...
                                                                      --query 'RouteTable.RouteTableId' \
                                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_private_rtba_id
...
1. **Create Private Route Table for Availability Zone B, Default Route and Associate with Private Subnets**
    ...
    alfa_ohio_production_private_rtbb_id=$(aws ec2 create-route-table --vpc-id $alfa_ohio_production_vpc_id \
                                                                      ...
                                                                      --query 'RouteTable.RouteTableId' \
                                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_private_rtbb_id
...
1. **Create Private Route Table for Availability Zone C, Default Route and Associate with Private Subnets**
    ...
    alfa_ohio_production_private_rtbc_id=$(aws ec2 create-route-table --vpc-id $alfa_ohio_production_vpc_id \
                                                                      ...
                                                                      --query 'RouteTable.RouteTableId' \
                                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_private_rtbc_id
...
1. **Create VPC Endpoint Security Group**
    ...
    alfa_ohio_production_vpce_sg_id=$(aws ec2 create-security-group --group-name Alfa-Production-VPCEndpointSecurityGroup \
                                                                    ...
                                                                    --query 'GroupId' \
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_vpce_sg_id
...
1. **Create VPC Endpoints for SSM and SSMMessages**
    ...
    alfa_ohio_production_ssm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $alfa_ohio_production_vpc_id \
                                                                   ...
                                                                   --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_ssm_vpce_id
    ...
    alfa_ohio_production_ssmm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $alfa_ohio_production_vpc_id \
                                                                    ...
                                                                    --query 'VpcEndpoint.VpcEndpointId' \
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable alfa_ohio_production_ssmm_vpce_id
    ```
