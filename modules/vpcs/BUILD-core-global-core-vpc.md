# Modules:VPCs:Core Account:Global:Core VPC
...
1. **Create VPC**
    ...
    camelz-variable global_core_vpc_id
...
1. **Tag Attached Default Resources Created With VPC**
    ...
    main_rtb_id=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$global_core_vpc_id \
    ...
    default_nacl_id=$(aws ec2 describe-network-acls --filters Name=vpc-id,Values=$global_core_vpc_id \
    ...
    default_sg_id=$(aws ec2 describe-security-groups --filters Name=vpc-id,Values=$global_core_vpc_id \
...
1. **Create Internet Gateway**
    ...
    global_core_igw_id=$(aws ec2 create-internet-gateway --tag-specifications "ResourceType=internet-gateway,Tags=[{Key=Name,Value=Core-InternetGateway},{Key=Company,Value=CaMeLz},{Key=Environment,Value=Core},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                         ...
                                                         --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_igw_id
...
1. **Create Private Hosted Zone**
    ...
    camelz-variable global_core_private_hostedzone_id
    ```
...
1. **Create DHCP Options Set**
    ...
    global_core_dopt_id=$(aws ec2 create-dhcp-options --dhcp-configurations "Key=domain-name,Values=[$global_core_private_domain]" \
                                                      ...
                                                      --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_dopt_id
...
1. **Create Public Subnet A**
    ...
    global_core_public_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                          --cidr-block $global_core_public_subneta_cidr \
                                                          ...
                                                          --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public_subneta_id
    ```
...
1. **Create Public Subnet B**
    ...
    global_core_public_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                          --cidr-block $global_core_public_subnetb_cidr \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public_subnetb_id
    ```
...
1. **Create Public Subnet C**
    ...
    global_core_public_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                          --cidr-block $global_core_public_subnetc_cidr \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public_subnetc_id
    ```

1. **Create Public7 Subnet A**

    ```bash
    global_core_public7_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_public7_subneta_cidr \
                                                           --availability-zone us-east-1a \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public7_subneta_id
    ```

1. **Create Public7 Subnet B**

    ```bash
    global_core_public7_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_public7_subnetb_cidr \
                                                           --availability-zone us-east-1b \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public7_subnetb_id
    ```

1. **Create Public7 Subnet C**

    ```bash
    global_core_public7_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_public7_subnetc_cidr \
                                                           --availability-zone us-east-1c \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public7_subnetc_id
    ```
...
1. **Create Web Subnet A**
    ...
    global_core_web_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                       --cidr-block $global_core_web_subneta_cidr \
                                                       --availability-zone us-east-1a \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web_subneta_id
    ```
...
1. **Create Web Subnet B**
    ...
    global_core_web_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                       --cidr-block $global_core_web_subnetb_cidr \
                                                       --availability-zone us-east-1b \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web_subnetb_id
    ```
...
1. **Create Web Subnet C**
    ...
    global_core_web_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                       --cidr-block $global_core_web_subnetc_cidr \
                                                       --availability-zone us-east-1c \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web_subnetc_id
    ```

1. **Create Web7 Subnet A**

    ```bash
    global_core_web7_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                        --cidr-block $global_core_web7_subneta_cidr \
                                                        --availability-zone us-east-1a \
                                                        ...
                                                        --query 'Subnet.SubnetId' \
                                                        --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web7_subneta_id
    ```

1. **Create Web7 Subnet B**

    ```bash
    global_core_web7_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                        --cidr-block $global_core_web7_subnetb_cidr \
                                                        --availability-zone us-east-1b \
                                                        ...
                                                        --query 'Subnet.SubnetId' \
                                                        --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web7_subnetb_id
    ```

1. **Create Web7 Subnet C**

    ```bash
    global_core_web7_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                        --cidr-block $global_core_web7_subnetc_cidr \
                                                        --availability-zone us-east-1c \
                                                        ...
                                                        --query 'Subnet.SubnetId' \
                                                        --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_web7_subnetc_id
    ```
...
1. **Create Application Subnet A**
    ...
    global_core_application_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                               --cidr-block $global_core_application_subneta_cidr \
                                                               --availability-zone us-east-1a \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application_subneta_id
    ```
...
1. **Create Application Subnet B**
    ...
    global_core_application_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                               --cidr-block $global_core_application_subnetb_cidr \
                                                               --availability-zone us-east-1b \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application_subnetb_id
    ```
...
1. **Create Application Subnet C**
    ...
    global_core_application_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                               --cidr-block $global_core_application_subnetc_cidr \
                                                               --availability-zone us-east-1c \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application_subnetc_id
    ```

1. **Create Application1 Subnet A**

    ```bash
    global_core_application1_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                                --cidr-block $global_core_application1_subneta_cidr \
                                                                --availability-zone us-east-1a \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application1_subneta_id
    ```

1. **Create Application1 Subnet B**

    ```bash
    global_core_application1_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                                --cidr-block $global_core_application1_subnetb_cidr \
                                                                --availability-zone us-east-1b \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application1_subnetb_id
    ```

1. **Create Application1 Subnet C**

    ```bash
    global_core_application1_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                                --cidr-block $global_core_application1_subnetc_cidr \
                                                                --availability-zone us-east-1c \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_application1_subnetc_id
    ```

1. **Create Cache Subnet A**

    ```bash
    global_core_cache_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                         --cidr-block $global_core_cache_subneta_cidr \
                                                         --availability-zone us-east-1a \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_cache_subneta_id
    ```

1. **Create Cache Subnet B**

    ```bash
    global_core_cache_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                         --cidr-block $global_core_cache_subnetb_cidr \
                                                         --availability-zone us-east-1b \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_cache_subnetb_id
    ```

1. **Create Cache Subnet C**

    ```bash
    global_core_cache_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                         --cidr-block $global_core_cache_subnetc_cidr \
                                                         --availability-zone us-east-1c \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_cache_subnetc_id
    ```
...
1. **Create Database Subnet A**
    ...
    global_core_database_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_database_subneta_cidr \
                                                            --availability-zone us-east-1a \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_database_subneta_id
    ```
...
1. **Create Database Subnet B**
    ...
    global_core_database_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_database_subnetb_cidr \
                                                            --availability-zone us-east-1b \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_database_subnetb_id
    ```
...
1. **Create Database Subnet C**
    ...
    global_core_database_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_database_subnetc_cidr \
                                                            --availability-zone us-east-1c \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_database_subnetc_id
    ```
...
1. **Create Endpoint Subnet A**
    ...
    global_core_endpoint_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_endpoint_subneta_cidr \
                                                            --availability-zone us-east-1a \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_endpoint_subneta_id
    ```
...
1. **Create Endpoint Subnet B**
    ...
    global_core_endpoint_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_endpoint_subnetb_cidr \
                                                            --availability-zone us-east-1b \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_endpoint_subnetb_id
    ```
...
1. **Create Endpoint Subnet C**
    ...
    global_core_endpoint_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_endpoint_subnetc_cidr \
                                                            --availability-zone us-east-1c \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_endpoint_subnetc_id
    ```

1. **Create Firewall Subnet A**

    ```bash
    global_core_firewall_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_firewall_subneta_cidr \
                                                            --availability-zone us-east-1a \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_firewall_subneta_id
    ```

1. **Create Firewall Subnet B**

    ```bash
    global_core_firewall_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_firewall_subnetb_cidr \
                                                            --availability-zone us-east-1b \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_firewall_subnetb_id
    ```

1. **Create Firewall Subnet C**

    ```bash
    global_core_firewall_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                            --cidr-block $global_core_firewall_subnetc_cidr \
                                                            --availability-zone us-east-1c \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_firewall_subnetc_id
    ```
...
1. **Create Gateway Subnet A**
    ...
    global_core_gateway_subneta_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_gateway_subneta_cidr \
                                                           --availability-zone us-east-1a \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_gateway_subneta_id
    ```
...
1. **Create Gateway Subnet B**
    ...
    global_core_gateway_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_gateway_subnetb_cidr \
                                                           --availability-zone us-east-1b \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_gateway_subnetb_id
    ```
...
1. **Create Gateway Subnet C**
    ...
    global_core_gateway_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_core_vpc_id \
                                                           --cidr-block $global_core_gateway_subnetc_cidr \
                                                           --availability-zone us-east-1c \
                                                           ...
                                                           --query 'Subnet.SubnetId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_gateway_subnetc_id
    ```
...
1. **Create Public Route Table, Default Route and Associate with Public Subnets**
    ...
    global_core_public_rtb_id=$(aws ec2 create-route-table --vpc-id $global_core_vpc_id \
                                                           ...
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_public_rtb_id
    ...
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_public7_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_public7_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_public7_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_web7_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_web7_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_public_rtb_id --subnet-id $global_core_web7_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ```
...
1. **Create NAT Gateways - OR - NAT Instances**
    ...
    if [ $use_ngw = 1 ]; then
      ...
      global_core_ngw_eipa=$(aws ec2 allocate-address --domain vpc \
                                                      ...
                                                      --query 'AllocationId' \
                                                      --profile $profile --region us-east-1 --output text)
      camelz-variable global_core_ngw_eipa
      ...
      global_core_ngwa_id=$(aws ec2 create-nat-gateway --allocation-id $global_core_ngw_eipa \
                                                       ...
                                                       --profile $profile --region us-east-1 --output text)
      camelz-variable global_core_ngwa_id
      ...
      if [ $ha_ngw = 1 ]; then
        global_core_ngw_eipb=$(aws ec2 allocate-address --domain vpc \
                                                        ...
                                                        --query 'AllocationId' \
                                                        --profile $profile --region us-east-1 --output text)
        camelz-variable global_core_ngw_eipb
        ...
        global_core_ngwb_id=$(aws ec2 create-nat-gateway --allocation-id $global_core_ngw_eipb \
                                                         ...
                                                         --query 'NatGateway.NatGatewayId' \
                                                         --profile $profile --region us-east-1 --output text)
        camelz-variable global_core_ngwb_id
        ...
        global_core_ngw_eipc=$(aws ec2 allocate-address --domain vpc \
                                                        ...
                                                        --query 'AllocationId' \
                                                        --profile $profile --region us-east-1 --output text)
        camelz-variable global_core_ngw_eipc
        ...
        global_core_ngwc_id=$(aws ec2 create-nat-gateway --allocation-id $global_core_ngw_eipc \
                                                         ...
                                                         --query 'NatGateway.NatGatewayId' \
                                                         --profile $profile --region us-east-1 --output text)
        camelz-variable global_core_ngwc_id
      fi
    else
      ...
      global_core_nat_sg_id=$(aws ec2 create-security-group --group-name Core-NAT-InstanceSecurityGroup \
                                                            ...
                                                            --query 'GroupId' \
                                                            --profile $profile --region us-east-1 --output text)
      camelz-variable global_core_nat_sg_id
      ...
      # Create NAT Instance
      global_core_nat_instance_id=$(aws ec2 run-instances --image-id $global_nat_ami_id \
                                                          ...
                                                          --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=Core-NAT-Instance},{Key=Hostname,Value=cmlue1mnat01a},{Key=Company,Value=CaMeLz},{Key=Environment,Value=Core},{Key=Utility,Value=NAT},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                          --query 'Instances[0].InstanceId' \
                                                          --profile $profile --region us-east-1 --output text)
      camelz-variable global_core_nat_instance_id
...
1. **Create Private Route Table for Availability Zone A, Default Route and Associate with Private Subnets**
    ...
    global_core_private_rtba_id=$(aws ec2 create-route-table --vpc-id $global_core_vpc_id \
                                                             ...
                                                             --query 'RouteTable.RouteTableId' \
                                                             --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_private_rtba_id
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtba_id --subnet-id $global_core_application_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtba_id --subnet-id $global_core_application1_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtba_id --subnet-id $global_core_cache_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtba_id --subnet-id $global_core_endpoint_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtba_id --subnet-id $global_core_firewall_subneta_id \
                                  --profile $profile --region us-east-1 --output text
...
1. **Create Private Route Table for Availability Zone B, Default Route and Associate with Private Subnets**
    ...
    global_core_private_rtbb_id=$(aws ec2 create-route-table --vpc-id $global_core_vpc_id \
                                                             ...
                                                             --query 'RouteTable.RouteTableId' \
                                                             --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_private_rtbb_id
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbb_id --subnet-id $global_core_application_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbb_id --subnet-id $global_core_application1_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbb_id --subnet-id $global_core_cache_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbb_id --subnet-id $global_core_endpoint_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbb_id --subnet-id $global_core_firewall_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
...
1. **Create Private Route Table for Availability Zone C, Default Route and Associate with Private Subnets**
    ...
    global_core_private_rtbc_id=$(aws ec2 create-route-table --vpc-id $global_core_vpc_id \
                                                             ...
                                                             --query 'RouteTable.RouteTableId' \
                                                             --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_private_rtbc_id
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbc_id --subnet-id $global_core_application_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbc_id --subnet-id $global_core_application1_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbc_id --subnet-id $global_core_cache_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbc_id --subnet-id $global_core_endpoint_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_core_private_rtbc_id --subnet-id $global_core_firewall_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
...
1. **Create VPC Endpoint Security Group**
    ...
    global_core_vpce_sg_id=$(aws ec2 create-security-group --group-name Core-VPCEndpointSecurityGroup \
                                                           ...
                                                           --query 'GroupId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_vpce_sg_id
...
1. **Create VPC Endpoints for SSM and SSMMessages**
    ...
    global_core_ssm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $global_core_vpc_id \
                                                          ...
                                                          --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_ssm_vpce_id
    ...
    global_core_ssmm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $global_core_vpc_id \
                                                           ...
                                                           --query 'VpcEndpoint.VpcEndpointId' \
                                                           --profile $profile --region us-east-1 --output text)
    camelz-variable global_core_ssmm_vpce_id
    ```
