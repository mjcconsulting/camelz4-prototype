# Modules:VPCs:Core Account:Ohio:Core VPC
...
1. **Create VPC**
    ...
    camelz-variable ohio_core_vpc_id
...
1. **Tag Attached Default Resources Created With VPC**
    ...
    main_rtb_id=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$ohio_core_vpc_id \
    ...
    default_nacl_id=$(aws ec2 describe-network-acls --filters Name=vpc-id,Values=$ohio_core_vpc_id \
    ...
    default_sg_id=$(aws ec2 describe-security-groups --filters Name=vpc-id,Values=$ohio_core_vpc_id \
...
1. **Create VPC Flow Log**
    ...
    ```bash
    aws logs create-log-group --log-group-name "/$company_name_lc/$system_name_lc/FlowLog/Core/Ohio" \
                              --profile $profile --region us-east-2 --output text

    aws ec2 create-flow-logs --resource-type VPC --resource-ids $ohio_core_vpc_id \
                             --traffic-type ALL \
                             --log-destination-type cloud-watch-logs \
                             --log-destination "arn:aws:logs:us-east-2:${core_account_id}:log-group:/${company_name_lc}/${system_name_lc}/FlowLog/Core/Ohio" \
...
1. **Create Internet Gateway**
    ...
    ohio_core_igw_id=$(aws ec2 create-internet-gateway --tag-specifications "ResourceType=internet-gateway,Tags=[{Key=Name,Value=Core-InternetGateway},{Key=Company,Value=CaMeLz},{Key=Environment,Value=Core},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                       ...
                                                       --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_igw_id
...
1. **Create Private Hosted Zone**
    ...
    camelz-variable ohio_core_private_hostedzone_id
    ```
...
1. **Create DHCP Options Set**
    ...
    ohio_core_dopt_id=$(aws ec2 create-dhcp-options --dhcp-configurations "Key=domain-name,Values=[$ohio_core_private_domain]" \
                                                    ...
                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_dopt_id
...
1. **Create Public Subnet A**
    ...
    ohio_core_public_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                        --cidr-block $ohio_core_public_subneta_cidr \
                                                        ...
                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public_subneta_id
    ```
...
1. **Create Public Subnet B**
    ...
    ohio_core_public_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                        --cidr-block $ohio_core_public_subnetb_cidr \
                                                        ...
                                                        --query 'Subnet.SubnetId' \
                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public_subnetb_id
    ```
...
1. **Create Public Subnet C**
    ...
    ohio_core_public_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                        --cidr-block $ohio_core_public_subnetc_cidr \
                                                        ...
                                                        --query 'Subnet.SubnetId' \
                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public_subnetc_id
    ```

1. **Create Public7 Subnet A**

    ```bash
    ohio_core_public7_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_public7_subneta_cidr \
                                                         --availability-zone us-east-2a \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public7_subneta_id
    ```

1. **Create Public7 Subnet B**

    ```bash
    ohio_core_public7_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_public7_subnetb_cidr \
                                                         --availability-zone us-east-2b \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public7_subnetb_id
    ```

1. **Create Public7 Subnet C**

    ```bash
    ohio_core_public7_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_public7_subnetc_cidr \
                                                         --availability-zone us-east-2c \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public7_subnetc_id
    ```
...
1. **Create Web Subnet A**
    ...
    ohio_core_web_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                     --cidr-block $ohio_core_web_subneta_cidr \
                                                     --availability-zone us-east-2a \
                                                     ...
                                                     --query 'Subnet.SubnetId' \
                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web_subneta_id
    ```
...
1. **Create Web Subnet B**
    ...
    ohio_core_web_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                     --cidr-block $ohio_core_web_subnetb_cidr \
                                                     --availability-zone us-east-2b \
                                                     ...
                                                     --query 'Subnet.SubnetId' \
                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web_subnetb_id
    ```
...
1. **Create Web Subnet C**
    ...
    ohio_core_web_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                     --cidr-block $ohio_core_web_subnetc_cidr \
                                                     --availability-zone us-east-2c \
                                                     ...
                                                     --query 'Subnet.SubnetId' \
                                                     --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web_subnetc_id
    ```

1. **Create Web7 Subnet A**

    ```bash
    ohio_core_web7_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                      --cidr-block $ohio_core_web7_subneta_cidr \
                                                      --availability-zone us-east-2a \
                                                      ...
                                                      --query 'Subnet.SubnetId' \
                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web7_subneta_id
    ```

1. **Create Web7 Subnet B**

    ```bash
    ohio_core_web7_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                      --cidr-block $ohio_core_web7_subnetb_cidr \
                                                      --availability-zone us-east-2b \
                                                      ...
                                                      --query 'Subnet.SubnetId' \
                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web7_subnetb_id
    ```

1. **Create Web7 Subnet C**

    ```bash
    ohio_core_web7_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                      --cidr-block $ohio_core_web7_subnetc_cidr \
                                                      --availability-zone us-east-2c \
                                                      ...
                                                      --query 'Subnet.SubnetId' \
                                                      --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_web7_subnetc_id
    ```
...
1. **Create Application Subnet A**
    ...
    ohio_core_application_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                             --cidr-block $ohio_core_application_subneta_cidr \
                                                             --availability-zone us-east-2a \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application_subneta_id
    ```
...
1. **Create Application Subnet B**
    ...
    ohio_core_application_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                             --cidr-block $ohio_core_application_subnetb_cidr \
                                                             --availability-zone us-east-2b \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application_subnetb_id
    ```
...
1. **Create Application Subnet C**
    ...
    ohio_core_application_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                             --cidr-block $ohio_core_application_subnetc_cidr \
                                                             --availability-zone us-east-2c \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application_subnetc_id
    ```

1. **Create Application1 Subnet A**

    ```bash
    ohio_core_application1_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                              --cidr-block $ohio_core_application1_subneta_cidr \
                                                              --availability-zone us-east-2a \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application1_subneta_id
    ```

1. **Create Application1 Subnet B**

    ```bash
    ohio_core_application1_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                              --cidr-block $ohio_core_application1_subnetb_cidr \
                                                              --availability-zone us-east-2b \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application1_subnetb_id
    ```

1. **Create Application1 Subnet C**

    ```bash
    ohio_core_application1_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                              --cidr-block $ohio_core_application1_subnetc_cidr \
                                                              --availability-zone us-east-2c \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_application1_subnetc_id
    ```

1. **Create Cache Subnet A**

    ```bash
    ohio_core_cache_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                       --cidr-block $ohio_core_cache_subneta_cidr \
                                                       --availability-zone us-east-2a \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_cache_subneta_id
    ```

1. **Create Cache Subnet B**

    ```bash
    ohio_core_cache_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                       --cidr-block $ohio_core_cache_subnetb_cidr \
                                                       --availability-zone us-east-2b \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_cache_subnetb_id
    ```

1. **Create Cache Subnet C**

    ```bash
    ohio_core_cache_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                       --cidr-block $ohio_core_cache_subnetc_cidr \
                                                       --availability-zone us-east-2c \
                                                       ...
                                                       --query 'Subnet.SubnetId' \
                                                       --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_cache_subnetc_id
    ```
...
1. **Create Database Subnet A**
    ...
    ohio_core_database_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_database_subneta_cidr \
                                                          --availability-zone us-east-2a \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_database_subneta_id
    ```
...
1. **Create Database Subnet B**
    ...
    ohio_core_database_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_database_subnetb_cidr \
                                                          --availability-zone us-east-2b \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_database_subnetb_id
    ```
...
1. **Create Database Subnet C**
    ...
    ohio_core_database_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_database_subnetc_cidr \
                                                          --availability-zone us-east-2c \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_database_subnetc_id
    ```
...
1. **Create Endpoint Subnet A**
    ...
    ohio_core_endpoint_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_endpoint_subneta_cidr \
                                                          --availability-zone us-east-2a \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_endpoint_subneta_id
    ```
...
1. **Create Endpoint Subnet B**
    ...
    ohio_core_endpoint_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_endpoint_subnetb_cidr \
                                                          --availability-zone us-east-2b \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_endpoint_subnetb_id
    ```
...
1. **Create Endpoint Subnet C**
    ...
    ohio_core_endpoint_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_endpoint_subnetc_cidr \
                                                          --availability-zone us-east-2c \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_endpoint_subnetc_id
    ```

1. **Create Firewall Subnet A**

    ```bash
    ohio_core_firewall_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_firewall_subneta_cidr \
                                                          --availability-zone us-east-2a \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_firewall_subneta_id
    ```

1. **Create Firewall Subnet B**

    ```bash
    ohio_core_firewall_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_firewall_subnetb_cidr \
                                                          --availability-zone us-east-2b \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_firewall_subnetb_id
    ```

1. **Create Firewall Subnet C**

    ```bash
    ohio_core_firewall_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                          --cidr-block $ohio_core_firewall_subnetc_cidr \
                                                          --availability-zone us-east-2c \
                                                          ...
                                                          --query 'Subnet.SubnetId' \
                                                          --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_firewall_subnetc_id
    ```
...
1. **Create Gateway Subnet A**
    ...
    ohio_core_gateway_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_gateway_subneta_cidr \
                                                         --availability-zone us-east-2a \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_gateway_subneta_id
    ```
...
1. **Create Gateway Subnet B**
    ...
    ohio_core_gateway_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_gateway_subnetb_cidr \
                                                         --availability-zone us-east-2b \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_gateway_subnetb_id
    ```
...
1. **Create Gateway Subnet C**
    ...
    ohio_core_gateway_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_core_vpc_id \
                                                         --cidr-block $ohio_core_gateway_subnetc_cidr \
                                                         --availability-zone us-east-2c \
                                                         ...
                                                         --query 'Subnet.SubnetId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_gateway_subnetc_id
    ```
...
1. **Create Public Route Table, Default Route and Associate with Public Subnets**
    ...
    ohio_core_public_rtb_id=$(aws ec2 create-route-table --vpc-id $ohio_core_vpc_id \
                                                         ...
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_public_rtb_id
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_public7_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_public7_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_public7_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_web7_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_web7_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_public_rtb_id --subnet-id $ohio_core_web7_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ```
...
1. **Create NAT Gateways - OR - NAT Instances**
    ...
    if [ $use_ngw = 1 ]; then
      ...
      ohio_core_ngw_eipa=$(aws ec2 allocate-address --domain vpc \
                                                    ...
                                                    --query 'AllocationId' \
                                                    --profile $profile --region us-east-2 --output text)
      camelz-variable ohio_core_ngw_eipa
      ...
      ohio_core_ngwa_id=$(aws ec2 create-nat-gateway --allocation-id $ohio_core_ngw_eipa \
                                                     ...
                                                     --profile $profile --region us-east-2 --output text)
      camelz-variable ohio_core_ngwa_id
      ...
      if [ $ha_ngw = 1 ]; then
        ohio_core_ngw_eipb=$(aws ec2 allocate-address --domain vpc \
                                                      ...
                                                      --query 'AllocationId' \
                                                      --profile $profile --region us-east-2 --output text)
        camelz-variable ohio_core_ngw_eipb
        ...
        ohio_core_ngwb_id=$(aws ec2 create-nat-gateway --allocation-id $ohio_core_ngw_eipb \
                                                       ...
                                                       --query 'NatGateway.NatGatewayId' \
                                                       --profile $profile --region us-east-2 --output text)
        camelz-variable ohio_core_ngwb_id
        ...
        ohio_core_ngw_eipc=$(aws ec2 allocate-address --domain vpc \
                                                      ...
                                                      --query 'AllocationId' \
                                                      --profile $profile --region us-east-2 --output text)
        camelz-variable ohio_core_ngw_eipc
        ...
        ohio_core_ngwc_id=$(aws ec2 create-nat-gateway --allocation-id $ohio_core_ngw_eipc \
                                                       ...
                                                       --query 'NatGateway.NatGatewayId' \
                                                       --profile $profile --region us-east-2 --output text)
        camelz-variable ohio_core_ngwc_id
      fi
    else
      ...
      ohio_core_nat_sg_id=$(aws ec2 create-security-group --group-name Core-NAT-InstanceSecurityGroup \
                                                          ...
                                                          --query 'GroupId' \
                                                          --profile $profile --region us-east-2 --output text)
      camelz-variable ohio_core_nat_sg_id
      ...
      ohio_core_nat_instance_id=$(aws ec2 run-instances --image-id $ohio_nat_ami_id \
                                                        ...
                                                        --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=Core-NAT-Instance},{Key=Hostname,Value=cmlue2mnat01a},{Key=Company,Value=CaMeLz},{Key=Environment,Value=Core},{Key=Utility,Value=NAT},{Key=Project,Value=CaMeLz-POC-4}]" \
                                                        --query 'Instances[0].InstanceId' \
                                                        --profile $profile --region us-east-2 --output text)
      camelz-variable ohio_core_nat_instance_id
...
1. **Create Private Route Table for Availability Zone A, Default Route and Associate with Private Subnets**
    ...
    ohio_core_private_rtba_id=$(aws ec2 create-route-table --vpc-id $ohio_core_vpc_id \
                                                           ...
                                                           --query 'RouteTable.RouteTableId' \
                                                           --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_private_rtba_id
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtba_id --subnet-id $ohio_core_application_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtba_id --subnet-id $ohio_core_application1_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtba_id --subnet-id $ohio_core_cache_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtba_id --subnet-id $ohio_core_endpoint_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtba_id --subnet-id $ohio_core_firewall_subneta_id \
                                  --profile $profile --region us-east-2 --output text
...
1. **Create Private Route Table for Availability Zone B, Default Route and Associate with Private Subnets**
    ...
    ohio_core_private_rtbb_id=$(aws ec2 create-route-table --vpc-id $ohio_core_vpc_id \
                                                           ...
                                                           --query 'RouteTable.RouteTableId' \
                                                           --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_private_rtbb_id
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbb_id --subnet-id $ohio_core_application_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbb_id --subnet-id $ohio_core_application1_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbb_id --subnet-id $ohio_core_cache_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbb_id --subnet-id $ohio_core_endpoint_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbb_id --subnet-id $ohio_core_firewall_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
...
1. **Create Private Route Table for Availability Zone C, Default Route and Associate with Private Subnets**
    ...
    ohio_core_private_rtbc_id=$(aws ec2 create-route-table --vpc-id $ohio_core_vpc_id \
                                                           ...
                                                           --query 'RouteTable.RouteTableId' \
                                                           --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_private_rtbc_id
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbc_id --subnet-id $ohio_core_application_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbc_id --subnet-id $ohio_core_application1_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbc_id --subnet-id $ohio_core_cache_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbc_id --subnet-id $ohio_core_endpoint_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_core_private_rtbc_id --subnet-id $ohio_core_firewall_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
...
1. **Create VPC Endpoint Security Group**
    ...
    ohio_core_vpce_sg_id=$(aws ec2 create-security-group --group-name Core-VPCEndpointSecurityGroup \
                                                         ...
                                                         --query 'GroupId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_vpce_sg_id
...
1. **Create VPC Endpoints for SSM and SSMMessages**
    ...
    ohio_core_ssm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $ohio_core_vpc_id \
                                                        ...
                                                        --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_ssm_vpce_id
    ...
    ohio_core_ssmm_vpce_id=$(aws ec2 create-vpc-endpoint --vpc-id $ohio_core_vpc_id \
                                                         ...
                                                         --query 'VpcEndpoint.VpcEndpointId' \
                                                         --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_core_ssmm_vpce_id
    ```
