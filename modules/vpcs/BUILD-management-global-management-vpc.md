# Modules:VPCs:Management Account:Global:Management VPC
...
1. **Create Public Subnet A**
    ...
    global_management_public_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                --cidr-block $global_management_public_subneta_cidr \
...
1. **Create Public Subnet B**
    ...
    global_management_public_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                --cidr-block $global_management_public_subnetb_cidr \
...
1. **Create Public Subnet C**
    ...
    global_management_public_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                --cidr-block $global_management_public_subnetc_cidr \
...
1. **Create Public7 Subnet A**
    ...
    ```bash
    global_management_public7_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_public7_subneta_cidr \
                                                                 --availability-zone us-east-1a \
                                                                 ...
                                                                 --query 'Subnet.SubnetId' \
                                                                 --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_public7_subneta_id
    ```

1. **Create Public7 Subnet B**

    ```bash
    global_management_public7_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_public7_subnetb_cidr \
                                                                 --availability-zone us-east-1b \
                                                                 ...
                                                                 --query 'Subnet.SubnetId' \
                                                                 --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_public7_subnetb_id
    ```

1. **Create Public7 Subnet C**

    ```bash
    global_management_public7_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_public7_subnetc_cidr \
                                                                 --availability-zone us-east-1c \
                                                                 ...
                                                                 --query 'Subnet.SubnetId' \
                                                                 --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_public7_subnetc_id
    ```
...
1. **Create Web Subnet A**
    ...
    global_management_web_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                             --cidr-block $global_management_web_subneta_cidr \
                                                             --availability-zone us-east-1a \
...
1. **Create Web Subnet B**
    ...
    global_management_web_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                             --cidr-block $global_management_web_subnetb_cidr \
                                                             --availability-zone us-east-1b \
...
1. **Create Web Subnet C**
    ...
    global_management_web_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                             --cidr-block $global_management_web_subnetc_cidr \
                                                             --availability-zone us-east-1c \
...
1. **Create Web7 Subnet A**
    ...
    ```bash
    global_management_web7_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                              --cidr-block $global_management_web7_subneta_cidr \
                                                              --availability-zone us-east-1a \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_web7_subneta_id
    ```

1. **Create Web7 Subnet B**

    ```bash
    global_management_web7_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                              --cidr-block $global_management_web7_subnetb_cidr \
                                                              --availability-zone us-east-1b \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_web7_subnetb_id
    ```

1. **Create Web7 Subnet C**

    ```bash
    global_management_web7_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                              --cidr-block $global_management_web7_subnetc_cidr \
                                                              --availability-zone us-east-1c \
                                                              ...
                                                              --query 'Subnet.SubnetId' \
                                                              --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_web7_subnetc_id
    ```
...
1. **Create Application Subnet A**
    ...
    global_management_application_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                     --cidr-block $global_management_application_subneta_cidr \
                                                                     --availability-zone us-east-1a \
...
1. **Create Application Subnet B**
    ...
    global_management_application_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                     --cidr-block $global_management_application_subnetb_cidr \
                                                                     --availability-zone us-east-1b \
...
1. **Create Application Subnet C**
    ...
    global_management_application_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                     --cidr-block $global_management_application_subnetc_cidr \
                                                                     --availability-zone us-east-1c \
...
1. **Create Application1 Subnet A**
    ...
    ```bash
    global_management_application1_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                      --cidr-block $global_management_application1_subneta_cidr \
                                                                      --availability-zone us-east-1a \
                                                                      ...
                                                                      --query 'Subnet.SubnetId' \
                                                                      --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_application1_subneta_id
    ```

1. **Create Application1 Subnet B**

    ```bash
    global_management_application1_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                      --cidr-block $global_management_application1_subnetb_cidr \
                                                                      --availability-zone us-east-1b \
                                                                      ...
                                                                      --query 'Subnet.SubnetId' \
                                                                      --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_application1_subnetb_id
    ```

1. **Create Application1 Subnet C**

    ```bash
    global_management_application1_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                      --cidr-block $global_management_application1_subnetc_cidr \
                                                                      --availability-zone us-east-1c \
                                                                      ...
                                                                      --query 'Subnet.SubnetId' \
                                                                      --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_application1_subnetc_id
    ```

1. **Create Cache Subnet A**

    ```bash
    global_management_cache_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                               --cidr-block $global_management_cache_subneta_cidr \
                                                               --availability-zone us-east-1a \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_cache_subneta_id
    ```

1. **Create Cache Subnet B**

    ```bash
    global_management_cache_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                               --cidr-block $global_management_cache_subnetb_cidr \
                                                               --availability-zone us-east-1b \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_cache_subnetb_id
    ```

1. **Create Cache Subnet C**

    ```bash
    global_management_cache_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                               --cidr-block $global_management_cache_subnetc_cidr \
                                                               --availability-zone us-east-1c \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_cache_subnetc_id
    ```
...
1. **Create Database Subnet A**
    ...
    global_management_database_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_database_subneta_cidr \
                                                                  --availability-zone us-east-1a \
...
1. **Create Database Subnet B**
    ...
    global_management_database_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_database_subnetb_cidr \
                                                                  --availability-zone us-east-1b \
...
1. **Create Database Subnet C**
    ...
    global_management_database_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_database_subnetc_cidr \
                                                                  --availability-zone us-east-1c \
...
1. **Create Directory Subnet A**
    ...
    global_management_directory_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                   --cidr-block $global_management_directory_subneta_cidr \
                                                                   --availability-zone us-east-1a \
...
1. **Create Directory Subnet B**
    ...
    global_management_directory_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                   --cidr-block $global_management_directory_subnetb_cidr \
                                                                   --availability-zone us-east-1b \
...
1. **Create Directory Subnet C**
    ...
    global_management_directory_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                   --cidr-block $global_management_directory_subnetc_cidr \
                                                                   --availability-zone us-east-1c \
...
1. **Create Endpoint Subnet A**
    ...
    global_management_endpoint_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_endpoint_subneta_cidr \
                                                                  --availability-zone us-east-1a \
...
1. **Create Endpoint Subnet B**
    ...
    global_management_endpoint_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_endpoint_subnetb_cidr \
                                                                  --availability-zone us-east-1b \
...
1. **Create Endpoint Subnet C**
    ...
    global_management_endpoint_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_endpoint_subnetc_cidr \
                                                                  --availability-zone us-east-1c \
...
1. **Create Firewall Subnet A**
    ...
    ```bash
    global_management_firewall_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_firewall_subneta_cidr \
                                                                  --availability-zone us-east-1a \
                                                                  ...
                                                                  --query 'Subnet.SubnetId' \
                                                                  --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_firewall_subneta_id
    ```

1. **Create Firewall Subnet B**

    ```bash
    global_management_firewall_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_firewall_subnetb_cidr \
                                                                  --availability-zone us-east-1b \
                                                                  ...
                                                                  --query 'Subnet.SubnetId' \
                                                                  --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_firewall_subnetb_id
    ```

1. **Create Firewall Subnet C**

    ```bash
    global_management_firewall_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                  --cidr-block $global_management_firewall_subnetc_cidr \
                                                                  --availability-zone us-east-1c \
                                                                  ...
                                                                  --query 'Subnet.SubnetId' \
                                                                  --profile $profile --region us-east-1 --output text)
    camelz-variable global_management_firewall_subnetc_id
    ```
...
1. **Create Gateway Subnet A**
    ...
    global_management_gateway_subneta_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_gateway_subneta_cidr \
                                                                 --availability-zone us-east-1a \
...
1. **Create Gateway Subnet B**
    ...
    global_management_gateway_subnetb_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_gateway_subnetb_cidr \
                                                                 --availability-zone us-east-1b \
...
1. **Create Gateway Subnet C**
    ...
    global_management_gateway_subnetc_id=$(aws ec2 create-subnet --vpc-id $global_management_vpc_id \
                                                                 --cidr-block $global_management_gateway_subnetc_cidr \
                                                                 --availability-zone us-east-1c \
...
1. **Create Public Route Table, Default Route and Associate with Public Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_public7_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_public7_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_public7_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_web7_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_web7_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_public_rtb_id --subnet-id $global_management_web7_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ```
...
1. **Create Private Route Table for Availability Zone A, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtba_id --subnet-id $global_management_application_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtba_id --subnet-id $global_management_application1_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtba_id --subnet-id $global_management_cache_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtba_id --subnet-id $global_management_endpoint_subneta_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtba_id --subnet-id $global_management_firewall_subneta_id \
                                  --profile $profile --region us-east-1 --output text
...
1. **Create Private Route Table for Availability Zone B, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbb_id --subnet-id $global_management_application_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbb_id --subnet-id $global_management_application1_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbb_id --subnet-id $global_management_cache_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbb_id --subnet-id $global_management_endpoint_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbb_id --subnet-id $global_management_firewall_subnetb_id \
                                  --profile $profile --region us-east-1 --output text
...
1. **Create Private Route Table for Availability Zone C, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbc_id --subnet-id $global_management_application_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbc_id --subnet-id $global_management_application1_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbc_id --subnet-id $global_management_cache_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    ...
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbc_id --subnet-id $global_management_endpoint_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
    aws ec2 associate-route-table --route-table-id $global_management_private_rtbc_id --subnet-id $global_management_firewall_subnetc_id \
                                  --profile $profile --region us-east-1 --output text
