# Modules:VPCs:Management Account:Ohio:Management VPC
...
1. **Create Public Subnet A**
    ...
    ohio_management_public_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                              --cidr-block $ohio_management_public_subneta_cidr \
...
1. **Create Public Subnet B**
    ...
    ohio_management_public_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                              --cidr-block $ohio_management_public_subnetb_cidr \
...
1. **Create Public Subnet C**
    ...
    ohio_management_public_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                              --cidr-block $ohio_management_public_subnetc_cidr \
...
1. **Create Public7 Subnet A**
    ...
    ```bash
    ohio_management_public7_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_public7_subneta_cidr \
                                                               --availability-zone us-east-2a \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_public7_subneta_id
    ```

1. **Create Public7 Subnet B**

    ```bash
    ohio_management_public7_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_public7_subnetb_cidr \
                                                               --availability-zone us-east-2b \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_public7_subnetb_id
    ```

1. **Create Public7 Subnet C**

    ```bash
    ohio_management_public7_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_public7_subnetc_cidr \
                                                               --availability-zone us-east-2c \
                                                               ...
                                                               --query 'Subnet.SubnetId' \
                                                               --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_public7_subnetc_id
    ```
...
1. **Create Web Subnet A**
    ...
    ohio_management_web_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                           --cidr-block $ohio_management_web_subneta_cidr \
                                                           --availability-zone us-east-2a \
...
1. **Create Web Subnet B**
    ...
    ohio_management_web_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                           --cidr-block $ohio_management_web_subnetb_cidr \
                                                           --availability-zone us-east-2b \
...
1. **Create Web Subnet C**
    ...
    ohio_management_web_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                           --cidr-block $ohio_management_web_subnetc_cidr \
                                                           --availability-zone us-east-2c \
...
1. **Create Web7 Subnet A**
    ...
    ```bash
    ohio_management_web7_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                            --cidr-block $ohio_management_web7_subneta_cidr \
                                                            --availability-zone us-east-2a \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_web7_subneta_id
    ```

1. **Create Web7 Subnet B**

    ```bash
    ohio_management_web7_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                            --cidr-block $ohio_management_web7_subnetb_cidr \
                                                            --availability-zone us-east-2b \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_web7_subnetb_id
    ```

1. **Create Web7 Subnet C**

    ```bash
    ohio_management_web7_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                            --cidr-block $ohio_management_web7_subnetc_cidr \
                                                            --availability-zone us-east-2c \
                                                            ...
                                                            --query 'Subnet.SubnetId' \
                                                            --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_web7_subnetc_id
    ```
...
1. **Create Application Subnet A**
    ...
    ohio_management_application_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                   --cidr-block $ohio_management_application_subneta_cidr \
                                                                   --availability-zone us-east-2a \
...
1. **Create Application Subnet B**
    ...
    ohio_management_application_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                   --cidr-block $ohio_management_application_subnetb_cidr \
                                                                   --availability-zone us-east-2b \
...
1. **Create Application Subnet C**
    ...
    ohio_management_application_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                   --cidr-block $ohio_management_application_subnetc_cidr \
                                                                   --availability-zone us-east-2c \
...
1. **Create Application1 Subnet A**
    ...
    ```bash
    ohio_management_application1_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                    --cidr-block $ohio_management_application1_subneta_cidr \
                                                                    --availability-zone us-east-2a \
                                                                    ...
                                                                    --query 'Subnet.SubnetId' \
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_application1_subneta_id
    ```

1. **Create Application1 Subnet B**

    ```bash
    ohio_management_application1_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                    --cidr-block $ohio_management_application1_subnetb_cidr \
                                                                    --availability-zone us-east-2b \
                                                                    ...
                                                                    --query 'Subnet.SubnetId' \
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_application1_subnetb_id
    ```

1. **Create Application1 Subnet C**

    ```bash
    ohio_management_application1_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                    --cidr-block $ohio_management_application1_subnetc_cidr \
                                                                    --availability-zone us-east-2c \
                                                                    ...
                                                                    --query 'Subnet.SubnetId' \
                                                                    --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_application1_subnetc_id
    ```

1. **Create Cache Subnet A**

    ```bash
    ohio_management_cache_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                             --cidr-block $ohio_management_cache_subneta_cidr \
                                                             --availability-zone us-east-2a \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_cache_subneta_id
    ```

1. **Create Cache Subnet B**

    ```bash
    ohio_management_cache_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                             --cidr-block $ohio_management_cache_subnetb_cidr \
                                                             --availability-zone us-east-2b \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_cache_subnetb_id
    ```

1. **Create Cache Subnet C**

    ```bash
    ohio_management_cache_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                             --cidr-block $ohio_management_cache_subnetc_cidr \
                                                             --availability-zone us-east-2c \
                                                             ...
                                                             --query 'Subnet.SubnetId' \
                                                             --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_cache_subnetc_id
    ```
...
1. **Create Database Subnet A**
    ...
    ohio_management_database_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_database_subneta_cidr \
                                                                --availability-zone us-east-2a \
...
1. **Create Database Subnet B**
    ...
    ohio_management_database_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_database_subnetb_cidr \
                                                                --availability-zone us-east-2b \
...
1. **Create Database Subnet C**
    ...
    ohio_management_database_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_database_subnetc_cidr \
                                                                --availability-zone us-east-2c \
...
1. **Create Endpoint Subnet A**
    ...
    ohio_management_endpoint_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_endpoint_subneta_cidr \
                                                                --availability-zone us-east-2a \
...
1. **Create Endpoint Subnet B**
    ...
    ohio_management_endpoint_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_endpoint_subnetb_cidr \
                                                                --availability-zone us-east-2b \
...
1. **Create Endpoint Subnet C**
    ...
    ohio_management_endpoint_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_endpoint_subnetc_cidr \
                                                                --availability-zone us-east-2c \
...
1. **Create Firewall Subnet A**
    ...
    ```bash
    ohio_management_firewall_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_firewall_subneta_cidr \
                                                                --availability-zone us-east-2a \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_firewall_subneta_id
    ```

1. **Create Firewall Subnet B**

    ```bash
    ohio_management_firewall_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_firewall_subnetb_cidr \
                                                                --availability-zone us-east-2b \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_firewall_subnetb_id
    ```

1. **Create Firewall Subnet C**

    ```bash
    ohio_management_firewall_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                                --cidr-block $ohio_management_firewall_subnetc_cidr \
                                                                --availability-zone us-east-2c \
                                                                ...
                                                                --query 'Subnet.SubnetId' \
                                                                --profile $profile --region us-east-2 --output text)
    camelz-variable ohio_management_firewall_subnetc_id
    ```
...
1. **Create Gateway Subnet A**
    ...
    ohio_management_gateway_subneta_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_gateway_subneta_cidr \
                                                               --availability-zone us-east-2a \
...
1. **Create Gateway Subnet B**
    ...
    ohio_management_gateway_subnetb_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_gateway_subnetb_cidr \
                                                               --availability-zone us-east-2b \
...
1. **Create Gateway Subnet C**
    ...
    ohio_management_gateway_subnetc_id=$(aws ec2 create-subnet --vpc-id $ohio_management_vpc_id \
                                                               --cidr-block $ohio_management_gateway_subnetc_cidr \
                                                               --availability-zone us-east-2c \
...
1. **Create Public Route Table, Default Route and Associate with Public Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_public7_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_public7_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_public7_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_web7_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_web7_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_public_rtb_id --subnet-id $ohio_management_web7_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ```
...
1. **Create Private Route Table for Availability Zone A, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtba_id --subnet-id $ohio_management_application_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtba_id --subnet-id $ohio_management_application1_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtba_id --subnet-id $ohio_management_cache_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtba_id --subnet-id $ohio_management_endpoint_subneta_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtba_id --subnet-id $ohio_management_firewall_subneta_id \
                                  --profile $profile --region us-east-2 --output text
...
1. **Create Private Route Table for Availability Zone B, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbb_id --subnet-id $ohio_management_application_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbb_id --subnet-id $ohio_management_application1_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbb_id --subnet-id $ohio_management_cache_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbb_id --subnet-id $ohio_management_endpoint_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbb_id --subnet-id $ohio_management_firewall_subnetb_id \
                                  --profile $profile --region us-east-2 --output text
...
1. **Create Private Route Table for Availability Zone C, Default Route and Associate with Private Subnets**
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbc_id --subnet-id $ohio_management_application_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbc_id --subnet-id $ohio_management_application1_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbc_id --subnet-id $ohio_management_cache_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    ...
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbc_id --subnet-id $ohio_management_endpoint_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
    aws ec2 associate-route-table --route-table-id $ohio_management_private_rtbc_id --subnet-id $ohio_management_firewall_subnetc_id \
                                  --profile $profile --region us-east-2 --output text
